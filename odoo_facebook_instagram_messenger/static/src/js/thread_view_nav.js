...
export class ThreadViewNav extends Component {
    ...
    mounted() {
        ...
        if (
            this.threadViewNav &&
            this.threadViewNav.channel &&
            this.threadViewNav.channel.correspondent &&
            this.threadViewNav.channel.correspondent.persona.partner &&
            this.threadViewNav.channel.correspondent.persona.partner.user
        ) {
    ...
    render() {
        ...
        if (
            this.messaging &&
            this.messaging.wa_thread_view &&
            this.messaging.wa_thread_view.state.nav_active == "partner"
        ) {
    ...
    onClickMessenger() {
        ...
        setTimeout(function () {
            if (
                ...
                $(".o_ThreadView_messageList .o_MessageList_message")[
                    $(".o_ThreadView_messageList .o_MessageList_message").length - 1
                ] &&
                $(".o_ThreadView_messageList .o_MessageList_message")[
                    $(".o_ThreadView_messageList .o_MessageList_message").length - 1
                ].offsetTop
    ...
    //        }
    get threadViewNav() {
        var threads = this.messaging.models["Thread"]
            .all()
            .filter((thread) => thread.localId == this.props.localId);
...
Object.assign(ThreadViewNav, {
    ...
    props: {localId: String},
    template: "odoo_facebook_instagram_messenger.ThreadViewNav",
});
